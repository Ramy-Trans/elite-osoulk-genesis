...
// ─── Standalone HTML Page Renderer (/p/:slug bypasses SPA, served as raw HTML)
...
<style>body{font-family:system-ui,sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;background:#f5f7fa}
.box{text-align:center;padding:2rem}h1{color:#061E46;font-size:2rem;margin-bottom:.5rem}p{color:#666;margin-bottom:1.5rem}
a{display:inline-block;background:#061E46;color:#fff;text-decoration:none;padding:.6rem 1.5rem;border-radius:.75rem;font-weight:700}</style>
</head><body><div class="box"><h1>404 — الصفحة غير موجودة</h1><p>لم يتم العثور على هذه الصفحة.</p><a href="/">العودة للرئيسية</a></div></body></html>`);
    }
    const rawHtml = page.bodyCode || page.content || "";
    const headCode = page.headCode || "";
    const title = page.seoTitle || page.titleAr || page.title || "أصولك";
    const desc = (page.seoDescription || "").replace(/"/g, "&quot;");
    // If already a full HTML document, serve as-is
    if (/^\s*<!doctype/i.test(rawHtml) || /^\s*<html/i.test(rawHtml)) {
      return res.type("html").send(rawHtml);
    }
    // Wrap in a complete HTML document shell
...
app.put("/api/admin/pages/:id", requireAdmin, async (req, res) => {
  try {
    ...
    const idx = all.findIndex(p => p.id === req.params.id);
    if (idx === -1) return res.status(404).json({ message: "Not found" });
    const FIELDS = ["slug","title","titleAr","heroImage","heroTitle","heroTitleAr","content","contentAr","publishStatus","renderMode","seoTitle","seoDescription","seoKeywords","ogImage","headCode","bodyCode","showInNav","showInMenu","showInFooter"];
    for (const k of FIELDS) { if (req.body[k] !== undefined) all[idx][k] = req.body[k]; }
    all[idx].updatedAt = new Date().toISOString();
